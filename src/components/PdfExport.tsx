...
export default function PdfExport({ answers }: PdfExportProps) {
    ...
    const CoverSlide = () => (
        <div className="w-[1280px] h-[720px] flex font-serif bg-[#fdfbf7]">
            {/* Left Dark Side */}
    ...
    const QuestionSlide = ({ q }: { q: typeof questions[0] }) => (
        <div className="w-[1280px] h-[720px] flex font-serif bg-[#fdfbf7]">
    ...
    const generatePdf = async () => {
        ...
        try {
            ...
            const totalSlides = questions.length + 1; // Cover + 40 questions
            ...
            for (let i = 0; i < totalSlides; i++) {
                ...
                const element = sourceRef.current.firstElementChild as HTMLElement;
                if (!element) continue;
        ...
        <>
            ...
            <div className="fixed top-0 left-0 -z-50 opacity-0 pointer-events-none">
                <div ref={sourceRef}>
                    {currentIndex === 0 ? (
                        ...
                        <QuestionSlide q={questions[currentIndex - 1]} />
                    )}
                </div>
            </div>
            ...
            {isGenerating && (
                <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black/90 backdrop-blur-sm">
                    <div className="text-white mb-8 text-xl font-light tracking-widest">
                        正在生成您的年度反思... {progress} / {questions.length + 1}
                    </div>
                    <div className="transform scale-[0.4] sm:scale-[0.6] lg:scale-[0.8] shadow-2xl transition-all duration-300">
                        {currentIndex === 0 ? (
                            <CoverSlide />
                        ) : (
                            <QuestionSlide q={questions[currentIndex - 1]} />
